...
public class PieceMarker extends ARObject {
    ...
    static final double POSE_TOLERANCE = 60.0;

    private boolean isCorrectPose;                  // variable to say if the current pose is the desired pose

    public boolean display;                        // do we display the drawing on this marker
    ...
    public PieceMarker(String name, String patternName, double markerWidth, double[] markerCenter, boolean _display, Model _model) {
        ...
        this.isCorrectPose = true;      // the marker start in the correct pose if it is the reference
...
//        this.isCorrectPose = true;
        if (!this.isVisible()) {
            ...
            double dist = distance.compute(this.oldPoseInReferenceCS, this.correctPoseInReferenceCS);
            if (dist < POSE_TOLERANCE) {
                this.isCorrectPose = true;
            } else {
                this.isCorrectPose = false;
            }
